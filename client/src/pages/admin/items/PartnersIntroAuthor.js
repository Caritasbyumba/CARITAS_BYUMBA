...
import { useFetchAllPartnersIntroQuery } from '../../../features/API/admin-api-slice';

const PartnersIntroAuthor = () => {
  ...
  const { data, isFetching, refetch } = useFetchAllPartnersIntroQuery();
  ...
  const [partnersIntroId, setPartnersIntroId] = useState('');

  const updateForm = useCallback(
    (partnersIntroId) => {
      setLoading(true);
      axios
        .get(`/api/partnersintro/${partnersIntroId}`, {
  ...
  const myData = useMemo(
    () =>
      data?.results
        ? data.results.map((partnersIntro, index) => {
            return {
              id: index + 1,
              description: partnersIntro.description[selectedLanguage],
              updatedBy: partnersIntro.updatedBy.name,
              updatedAt: partnersIntro.updatedAt,
              status: partnersIntro.isActive,
              _id: partnersIntro._id,
            };
          })
        : [],
  ...
  const handleAdd = useCallback(() => {
    if (
      ...
      axios
        .post('/api/partnersintro/add', formData, {
          headers: { Authorization: token },
        })
        .then((res) => {
          setLoading(false);
  ...
  const handleUpdate = useCallback(() => {
    if (
      ...
      axios
        .patch(`/api/partnersintro/${partnersIntroId}`, formData, {
          headers: { Authorization: token },
        })
        .then((res) => {
          setLoading(false);
  ...
  const handleArchive = useCallback(() => {
    ...
    setError(null);
    axios
      .patch(`/api/partnersintro/archive/${partnersIntroId}`, null, {
        headers: { Authorization: token },
      })
      .then((res) => {
        setLoading(false);
  ...
  }, [token, partnersIntroId, refetch]);

  const handleDelete = useCallback(() => {
    setLoading(true);
    setError(null);
    axios
      .delete(`/api/partnersintro/${partnersIntroId}`, {
        headers: { Authorization: token },
      })
      .then((res) => {
        setLoading(false);
    ...
    <div>
      ...
      >
        <CardTitle
          name={`${
            isUpdating
              ? t('Update partners introduction')
              : t('Add new partners introduction')
          }`}
          color="red"
        />
        ...
        <Button
          ...
          color="red"
          additional="mt-3"
          clicked={isUpdating ? () => handleUpdate(partnersIntroId) : handleAdd}
        />
      </Modal>
      <Modal
      ...
      >
        <CardTitle name={t('Delete partners introduction')} color="red" />
        <CardBody
          name={`${t(
            'Are you sure you want to delete this partners introduction?'
      ...
      <div className="w-70% m-auto py-10">
        <SectionTitle name={t('List of all partners introduction')} />
        {isFetching ? (
          <Spinner />
        ) : (
...
export default PartnersIntroAuthor;

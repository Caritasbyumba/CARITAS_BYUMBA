...
const checkPartner = (req, res, next) => {
  const partnerSchemas = Joi.object().keys({
    ...
    enQuote: Joi.string().trim().max().required().label('English quote'),
    frQuote: Joi.string().trim().max().required().label('French quote'),
    rwQuote: Joi.string().trim().max().required().label('Kinyarwanda quote'),
